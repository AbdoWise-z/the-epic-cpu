...
public class Main {
    ...
    private static List<Instruction> toCode(List<String> lines , Map<String,Integer> dataMapping , int offset){
        ...
        for (String line : lines){
            ...
            if (line.split(" ")[0].trim().equalsIgnoreCase(".ORG")){
                String[] sp = line.split(" ");
                if (sp.length > 2){
                    log.e("wrong usage for .ORG");
                    exit(9);
                }
                int val = Util.toInteger(sp[1].trim()) - offset;
                int codeSize = Instruction.getCodeSize(instructions);
                if (val < codeSize) { //not the fastest way to do it
                    log.e("you cannot ORG backwards");
                    exit(10);
                }

                int count = val - codeSize;
                //log.e("ORG : " + count);
    ...
    public static void main(String[] args){
        ...
        List<Instruction> intInstructions = toCode(intCode , new HashMap<>() , 0);
        ...
        List<Instruction> instructions = toCode(code, dataMapping , Instruction.getCodeSize(intInstructions) + CODE_INT_PADDING + INT_CODE_OFFSET);
